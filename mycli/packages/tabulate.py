...
def _html_row_with_attrs(celltag, cell_values, colwidths, colaligns):
    ...
    values_with_attrs = ["<{0}{1}>{2}</{0}>".format(
        celltag, alignment.get(a, ''), c) for c, a in
        zip(cell_values, colaligns)]
...
_table_formats = {"simple":
                  ...
                  "latex_booktabs":
                  TableFormat(
                      lineabove=partial(_latex_line_begin_tabular,
                                        booktabs=True),
...
def _more_generic(type1, type2):
    types = {_none_type: 0, _bool_type: 1, int: 2, float: 3, _binary_type: 4,
             _text_type: 5}
...
def tabulate(tabular_data, headers=(), tablefmt="simple",
    ...
    >>> print(tabulate([["spam$_9$", 41.9999], ["\\\\emph{eggs}", "451.0"]],
    ...                tablefmt="latex_raw"))
    \\begin{tabular}{lr}
    \\hline
     spam$_9$    &  41.9999 \\\\
...
def _format_table(fmt, headers, rows, colwidths, colaligns):
    ...
    if padded_headers:
        lines.append(_build_row(padded_headers, padded_widths, colaligns,
                                headerrow))
        if fmt.linebelowheader and "linebelowheader" not in hidden:
            lines.append(_build_line(padded_widths, colaligns,
                                     fmt.linebelowheader))

    if padded_rows and fmt.linebetweenrows and "linebetweenrows" not in hidden:
        # initial rows with a line below
        for row in padded_rows[:-1]:
            lines.append(_build_row(row, padded_widths, colaligns,
                                    fmt.datarow))
            lines.append(_build_line(padded_widths, colaligns,
                                     fmt.linebetweenrows))
        # the last row without a line below
        lines.append(_build_row(padded_rows[-1], padded_widths, colaligns,
                                fmt.datarow))
    else:
